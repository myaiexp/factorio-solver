// Sizing tests for `size_step`: the worked examples from the design, the
// lane-allocation search itself, and every refusal a single-product step can
// reach. These would keep passing unchanged if `place` did not exist.
use super::super::*;
use factorio_grid::prototype;

use crate::chain::{self, ChainGoal, MachinePolicy, Rate};
use crate::testsupport::{blue_belt, green_circuit_plan, hand_step, rate, step};
